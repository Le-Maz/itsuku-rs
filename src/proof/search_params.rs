...
pub trait SearchParams: Copy {
    /// The endianness configuration used by the solver/verifier.
    type SolverEndian: Endian;
    /// The specific implementation of partial memory access (Full or Sparse).
    type MemoryType: PartialMemory<Self::SolverEndian>;
    /// The specific implementation of partial Merkle tree access (Full or Sparse).
    type MerkleTreeType: PartialMerkleTree<Self::SolverEndian>;
...
#[derive(Clone, Copy)]
pub struct SolverSearchParams<'a, E: Endian> {
    /// The protocol configuration.
    pub config: &'a Config,
    /// The specific challenge being solved.
    pub challenge_id: &'a ChallengeId,
    /// The full memory dataset.
    pub memory: &'a Memory<E>,
    /// The complete Merkle tree commitment.
    pub merkle_tree: &'a MerkleTree<E>,
}

impl<'a, E: Endian> SearchParams for SolverSearchParams<'a, E> {
    type SolverEndian = E;
    type MemoryType = Memory<E>;
    type MerkleTreeType = MerkleTree<E>;

    fn config(&self) -> &Config { self.config }
    fn challenge_id(&self) -> &ChallengeId { self.challenge_id }
    fn memory(&self) -> &Self::MemoryType { self.memory }
    fn merkle_tree(&self) -> &Self::MerkleTreeType { self.merkle_tree }
}
...
#[derive(Clone, Copy)]
pub struct VerifierSearchParams<'a, E: Endian> {
    ...
    pub memory: &'a VerifierMemory<E>,
    /// The sparse Merkle tree containing only revealed nodes/paths.
    pub merkle_tree: &'a VerifierMerkleTree<E>,
}

impl<'a, E: Endian> SearchParams for VerifierSearchParams<'a, E> {
    type SolverEndian = E;
    type MemoryType = VerifierMemory<E>;
    type MerkleTreeType = VerifierMerkleTree<E>;
